...
4. Push another commit to main to trigger the deploy.

You only need to do steps 2 and 3 once. After that, every push to main auto-deploys.

## Install the updater on your OpenComputers computer
...
It will download all programs and libraries from your repo onto the computer. From now on, every time you push code the CI deploys it to Pages. Run `update` on your computer to pull the latest versions. It only downloads files that changed, so it is fast.

## Write programs

Place programs in `src/bin/` and libraries in `src/lib/`. The filesystem structure matches what OpenComputers expects.

## Check your code

Run `make lint` to check for errors. The CI does this automatically on every push.

## Running the updater on boot
